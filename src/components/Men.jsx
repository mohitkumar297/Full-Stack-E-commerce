...
function Men() {
    ...
    const fetchItems = async () => {
        const data = await fetch('/api/productsMen');
        const p = await data.json()
        setproducts(p)
    }

    const fetchCategories = async () => {
        const data = await fetch('/api/getCategoriesMen');
        const p = await data.json()
        setcategories(p)
    }
            ...
            <Dropdown>
                ...
                <Dropdown.Menu>
                    {categories.map(category => (
                        <Dropdown.Item key={category._id} onClick={handleDropdown}>{category.name}</Dropdown.Item>
                    ))}
                </Dropdown.Menu>
            </Dropdown>
